class KibF {
  ...
  getKIBFByNo(no, entity) {
    ...
    throw new Error(`KIB F dengan No ${no} tidak ditemukan`);
  }
  ...
  getBarang(){
    const sheet = this.sheet.getSheetByName('Barang-F');
    const lastRow = sheet.getLastRow();
    const lastColumn = sheet.getLastColumn();
    if (sheet) {
      const data = sheet.getRange(2, 1, lastRow - 1, lastColumn).getValues();
  ...
  getKelompokByBarang(barang){
    ...
    const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Kelompok-F");
  ...
  getSubKelompokByKelompok(kelompok){
    // Mendapatkan sheet aktif (ganti dengan nama sheet yang sesuai jika perlu)
    const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Sub Kelompok-F");
  ...
  getJenisBySubKelompok(subkelompok){
    // Mendapatkan sheet aktif (ganti dengan nama sheet yang sesuai jika perlu)
    const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Jenis-F");
  ...
  getJenis(){
    const sheet = this.sheet.getSheetByName('Jenis-F');
    const lastRow = sheet.getLastRow();
    const lastColumn = sheet.getLastColumn();
    if (sheet) {
      const data = sheet.getRange(2, 1, lastRow - 1, lastColumn).getValues();
    ...
    for (let i = 1; i < data.length; i++) {
      if (data[i][0] == barang.no) {
        if(data[i][2] !== barang.jenis_barang){
          ...
          this.sheet.getRange(i + 1, 7).setValue(barang.bertingkat);
          this.sheet.getRange(i + 1, 8).setValue(barang.beton);
          this.sheet.getRange(i + 1, 9).setValue(barang.panjang_km);
          this.sheet.getRange(i + 1, 10).setValue(barang.lebar_m);
          this.sheet.getRange(i + 1, 11).setValue(barang.luas_m2);
          this.sheet.getRange(i + 1, 12).setValue(barang.letak);
          this.sheet.getRange(i + 1, 13).setValue(barang.status_tanah);
          this.sheet.getRange(i + 1, 14).setValue(barang.kondisi);
          this.sheet.getRange(i + 1, 15).setValue(barang.asal_usul);
          this.sheet.getRange(i + 1, 16).setValue(barang.harga);
